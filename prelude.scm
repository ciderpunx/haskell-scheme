...
(define (vector-copy v)   (if (vector? v) v (error "vector-copy needs a vector to copy")))
(define (list->vector l)  (vector l))
(define (vector->list v)  (as-list v))
(define (vector-length v) 
  (if (vector? v)
    (length (vector->list v)) 
    (error "Non-vector argument to vector-length?")))
...
(define (list->string l)  
  (if (not (list? l))
    (error "Non-list given to list->string")
    (foldl (λ (a c) (string-append a (make-string 1 c))) "" l)))
(define (string-copy s) ((compose list->string string->list) s))
(define (string-length s) 
  (if (string? s) 
    (length (string->list s)) 
    (error "Non-string argument to string-length")))
(define (string-null? s)  (eq? 0 (string-length s)))
(define (string-ref c s)   (if (or (> (+ c 1) (string-length s)) (< c 0))
                             (error "Index out of bounds in string-ref call")
                             (if (= c 0) 
                                 (car (string->list s)) 
                                 (string-ref (- c 1) (list->string (cdr (string->list s)))))))
